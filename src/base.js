...
class FinVizScreener {
    ...
    async scan() {
        ...
        do {
            ...
            tickers = tickers.concat($(selector).map((i, el) => {
                const data = $(el).attr('data-boxover').match(/(?<=\sbody=\[)(.*)(?=\])/g)[0].match(/(?<=<div>)(.*)(?=<\/div>)/g)[0];
                const [sector, country, capitalization] = data.split('</b>')[1].split(' | ');

                return {
                    ticker: $(el).text().trim(),
                    name: data.match(/(?<=<b>)(.*)(?=<\/b>)/g)[0],
                    sector,
                    country,
                    capitalization
                };
